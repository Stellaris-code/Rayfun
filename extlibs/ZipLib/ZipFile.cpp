...
ZipArchive::Ptr ZipFile::Open(const std::string& zipPath, bool create_if_no)
{
    std::ifstream* zipFile = new std::ifstream();
    zipFile->open(zipPath, std::ios::binary);

    if (!zipFile->is_open() && create_if_no)
    {
        ...
        if (!zipFile->is_open())
        {
            throw std::runtime_error("cannot create '" + zipPath + "' !");
        }
    }
    else if (!zipFile->is_open())
    {
        throw std::runtime_error("cannot open '" + zipPath + "' !");
    }
